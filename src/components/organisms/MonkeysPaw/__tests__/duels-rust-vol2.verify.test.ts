/// Monkey's Paw RUST volume 2 content verifier — proves every duel's
/// cheat ladder behaves as designed against REAL rustc, using the same
...
/// `paw-rust-`, exactly one duel per rank 1-10 with the rank→tier
/// mapping (1-2 novice, 3-4 apprentice, 5-6 journeyman, 7-8 master,
/// 9-10 grandmaster), and 3-5 cheats per duel.
///
/// Gated behind PAW_VERIFY=1 because it shells out to rustc dozens of
/// times (a couple of minutes) — far too slow for the default
/// `npm test` loop. Run it whenever this volume's content changes:
///
///   PAW_VERIFY=1 npx vitest run src/components/organisms/MonkeysPaw/__tests__/duels-rust-vol2.verify.test.ts
...
import { RUST_DUELS_VOL2 } from "@/components/organisms/MonkeysPaw/duels/rust-vol2";
...
function suitePasses(code: string, tests: string, label: string): boolean {
  const dir = mkdtempSync(join(tmpdir(), "paw-verify-rust-vol2-"));
  try {
...
describe.skipIf(!process.env.PAW_VERIFY)("Monkey's Paw rust duels (volume 2)", () => {
  it("ships a coherent 10-duel ladder", () => {
    expect(RUST_DUELS_VOL2.length).toBe(10);
    expect(new Set(RUST_DUELS_VOL2.map((d) => d.id)).size).toBe(10);
    // Exactly one duel per rank 1..10.
    expect(new Set(RUST_DUELS_VOL2.map((d) => d.rank)).size).toBe(10);
    for (const duel of RUST_DUELS_VOL2) {
