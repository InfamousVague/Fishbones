...
/// One fetched documentation page for the floating Docs window —
/// the same fetch + main-content-selector + html2md pipeline the
/// crawler uses, for a single page on demand. The frontend renders
/// the markdown through the app's lesson renderer so external docs
/// (MDN, doc.rust-lang.org, …) appear in Libre's own theme.
#[derive(Debug, Serialize)]
pub struct DocPage {
    pub url: String,
    pub title: String,
...
/// Fetch + extract a single docs page. URL scheme is validated here;
/// WHICH hosts are reachable is governed by the frontend's curated
/// booklet registry (it only navigates links within each booklet's
/// allow-listed hosts), mirroring how `crawl_docs_site` trusts its
/// caller-supplied start URL.
#[tauri::command]
pub async fn fetch_doc_page(url: String) -> Result<DocPage, String> {
    ...
    let extracted = fetch_and_extract(&client, &parsed).await?;
    Ok(DocPage {
        url: strip_fragment_string(&parsed),
        title: extracted.title,
